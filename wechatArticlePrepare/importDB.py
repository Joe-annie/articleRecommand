...
for doc in collection.find():
    ...
    doc['content_forsearch'] = article_search
    # get tfidf matrix
    article_cut = cutWords(txt_str, stop)
    txtlist_cut.append(article_cut)
    # update
    condition = {'_id':doc['_id']}
    result = collection.update_one(condition,{'$set':doc})
tfi(10)
for idx,doc in enumerate(collection.find()):
    doc['keywords'] = ' '.join(list(featurelist[idx]))
    condition = {'_id':doc['_id']}
    result = collection.update_one(condition,{'$set':doc})
